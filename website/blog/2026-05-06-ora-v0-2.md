...
A pool contract that uses most of v0.2's surface lives at [`ora-example/dex/fixed_rate_pool.ora`](https://github.com/oralang/Ora/blob/v0.2/ora-example/dex/fixed_rate_pool.ora). It's a fixed-rate pool with internal balance accounting, multi-error returns, refinement-typed parameters, ghost state, and a verified pure helper. `ora build` produces bytecode + source map + SMT report.
...
*v0.2 ships from the `v0.2` branch. Try it: clone the repo, `zig build install`, then `ora build ora-example/dex/fixed_rate_pool.ora` for the example above. Feedback and bug reports welcome on GitHub.*
