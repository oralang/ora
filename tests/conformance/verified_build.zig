...
const schema = "verified-conformance-manifest-v1";
...
test "verified conformance manifest pins all three build classes" {
    ...
    for (manifest.entries, specs) |entry, spec_name| {
        ...
        if (observed != entry.class) {
            std.debug.print("verified conformance class drift: {s}: manifest={s}, observed={s}\n", .{
                spec_name,
                @tagName(entry.class),
                @tagName(observed),
            });
...
test "resource rewrite executes with proved runtime checks kept" {
    ...
    try testing.expectEqual(
        ...
        try runner.classifyVerifiedBuild(testing.allocator, source_path, spec_path),
    );
...
fn parseManifestEntries(allocator: std.mem.Allocator, text: []const u8) ![]ManifestEntry {
    ...
    while (lines.next()) |raw_line| {
        ...
        const separator = std.mem.indexOfScalar(u8, line, '\t') orelse return error.InvalidManifestEntry;
        if (std.mem.indexOfScalar(u8, line[separator + 1 ..], '\t') != null) return error.InvalidManifestEntry;
        const class_text = line[0..separator];
        const spec_name = line[separator + 1 ..];
        ...
        try entries.append(allocator, .{ .class = class, .spec_name = spec_name });
    }
