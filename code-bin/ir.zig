...
/// Unified identifier type for all IR names (aliases TokenValue from lexer)
pub const IdentifierName = @import("lexer.zig").TokenValue;
...
pub const Type = union(enum) {
    ...
    module: ModuleType, // Add module type support
    ...
    pub const StructType = struct {
        ...
        /// Reference to the original type definition, if available.
        origin_type: ?*const Typer.StructType,
    ...
    pub fn deinit(self: *Type, allocator: Allocator) void {
        switch (self.*) {
            ...
            .module => |*module| {
                for (module.fields) |*field| {
                    field.field_type.deinit(allocator);
                    allocator.destroy(field.field_type);
                }
                allocator.free(module.fields);
            },
        }
    }
    ...
    pub fn isCompatibleWith(self: *const Type, other: *const Type) bool {
        ...
        return switch (self.*) {
            ...
            .custom => |c1| switch (other.*) {
                .custom => |c2| std.mem.eql(u8, c1.name.string, c2.name.string),
                else => false,
            },
            ...
            .struct_type => |struct_type1| switch (other.*) {
                .struct_type => |struct_type2| {
                    ...
                    return std.mem.eql(u8, struct_type1.name.string, struct_type2.name.string);
                },
                else => false,
            },
            .enum_type => |enum_type1| switch (other.*) {
                .enum_type => |enum_type2| {
                    ...
                    return std.mem.eql(u8, enum_type1.name.string, enum_type2.name.string);
                },
                else => false,
            },
            .module => |module1| switch (other.*) {
                .module => |module2| {
                    // Module types are compatible if they have the same name
                    return std.mem.eql(u8, module1.name.string, module2.name.string);
                },
                else => false,
            },
        };
    ...
    pub fn isEqual(self: *const Type, other: *const Type) bool {
        ...
        return switch (self.*) {
            ...
            .struct_type => |s1| blk: {
                ...
                if (!std.mem.eql(u8, s1.name.string, s2.name.string)) break :blk false;
                if (s1.fields.len != s2.fields.len) break :blk false;

                for (s1.fields, s2.fields) |f1, f2| {
                    if (!std.mem.eql(u8, f1.name.string, f2.name.string)) break :blk false;
            ...
            .custom => |c1| std.mem.eql(u8, c1.name.string, other.custom.name.string),
            else => false,
        };
    }
...
pub const EffectSet = struct {
    ...
    fn selectorsEqual(s1: AccessPath.PathSelector, s2: AccessPath.PathSelector) bool {
        return switch (s1) {
            .field => |f1| switch (s2) {
                .field => |f2| std.mem.eql(u8, f1.name.string, f2.name.string),
                .index => false,
            },
...
pub const Import = struct {
    ...
    pub fn deinit(self: *Import) void {
        _ = self; // No dynamic memory to free for now
    }
};
...
pub const HIRProgram = struct {
    ...
    pub fn addImport(self: *HIRProgram, import: Import) !void {
        std.debug.print("Adding import to HIR program: {s}\n", .{import.name});
        const new_imports = try self.allocator.realloc(self.imports, self.imports.len + 1);
        new_imports[new_imports.len - 1] = import;
        self.imports = new_imports;
        std.debug.print("HIR program now has {} imports\n", .{self.imports.len});
    }
};
...
pub const SwitchStatement = struct {
    ...
    pub fn deinit(self: *SwitchStatement) void {
        self.condition.deinit();
        for (self.cases) |*case| {
            case.deinit();
        }
...
pub const SwitchCase = struct {
    pattern: SwitchPatternHIR,
    body: Block,
    location: SourceLocation,
    allocator: Allocator,

    pub fn deinit(self: *SwitchCase) void {
        self.pattern.deinit(self.allocator);
        self.body.deinit();
    }
};

pub const SwitchPatternHIR = union(enum) {
    literal: *Expression,
    range: struct {
        start: *Expression,
        end: *Expression,
    ...
    pub fn deinit(self: *SwitchPatternHIR, allocator: Allocator) void {
        switch (self.*) {
            .literal => |*expr| {
                expr.deinit();
...
pub const SwitchExpression = struct {
    ...
    pub fn deinit(self: *SwitchExpression) void {
        self.condition.deinit();
        for (self.cases) |*case| {
            case.deinit();
        }
...
pub const Validator = struct {
    ...
    pub fn validateProgram(self: *Validator, program: *const HIRProgram) !ValidationResult {
        // Register imported modules in symbol table
        for (program.imports) |*import| {
            std.debug.print("Registering import in symbol table: {s}\n", .{import.name});
            self.symbol_table.addSymbol(SymbolTable.Symbol{
                .name = import.name,
                .type = Type{ .module = Type.ModuleType{ .name = IdentifierName{ .string = import.name }, .fields = &[_]Type.ModuleField{} } },
                .region = .stack,
                .mutable = false,
                .initialized = true,
                .kind = .module,
                ...
                if (err == error.DuplicateSymbol) {
                    try self.context.addError(ValidationError.withStaticMessage("Duplicate import name", import.location, .symbol_error));
                } else {
                    return err;
                }
            };
    ...
    fn validateContractStructure(self: *Validator, contract: *const Contract) anyerror!void {
        ...
        var has_init = false;
        for (contract.functions) |*function| {
            if (std.mem.eql(u8, function.name.string, "init")) {
                has_init = true;
                break;
            }
        }
        ...
        for (contract.events, 0..) |*event1, i| {
            for (contract.events[(i + 1)..]) |*event2| {
                if (std.mem.eql(u8, event1.name.string, event2.name.string)) {
    ...
    fn validateFunctionSignature(self: *Validator, function: *const Function) anyerror!void {
        ...
        for (function.parameters, 0..) |*param1, i| {
            for (function.parameters[(i + 1)..]) |*param2| {
                if (std.mem.eql(u8, param1.name.string, param2.name.string)) {
    ...
    fn validateStatement(self: *Validator, stmt: *const Statement) anyerror!void {
        switch (stmt.*) {
            ...
            .switch_statement => |*ss| try self.validateSwitchStatement(ss),
    ...
    fn getExpressionType(self: *const Validator, expr: *const Expression) ?Type {
        switch (expr.*) {
            ...
            .switch_expr => |*switch_expr| {
                // Switch expression type is inferred from case expressions
                if (switch_expr.cases.len > 0) {
                    // Get type of first case (all cases should have same type)
                    return self.getExpressionType(switch_expr.cases[0].body.statements[0]);
                }
                if (switch_expr.default_case) |default| {
                    return self.getExpressionType(default);
                }
                return null;
            },
        }
    ...
    fn inferBinaryExpressionType(self: *const Validator, binary: *const BinaryExpression) ?Type {
        ...
        switch (binary.operator) {
            ...
            // Exponentiation returns the left operand type
            .star_star => {
                return left_type;
            },
            // Comma operator returns the right operand type
            .comma => {
                return right_type;
            },
        }
    ...
    fn inferFieldExpressionType(self: *const Validator, field: *const FieldExpression) ?Type {
        ...
        if (target_type.? == .custom) {
            if (std.mem.eql(u8, target_type.?.custom.name.string, "std")) {
    ...
    fn validateSwitchStatement(self: *Validator, switch_stmt: *const SwitchStatement) anyerror!void {
        try self.validateExpression(switch_stmt.condition);

        // Validate each case
        for (switch_stmt.cases) |*case| {
            try self.validateSwitchCase(case);
        }

        // Validate default case if present
        if (switch_stmt.default_case) |*default| {
            try self.validateBlock(default);
        }

        // Check for exhaustiveness (basic check for now)
        if (switch_stmt.default_case == null) {
            const has_else_pattern = for (switch_stmt.cases) |*case| {
                if (case.pattern == .else_pattern) break true;
            } else false;

            if (!has_else_pattern) {
                try self.context.addError(ValidationError.withStaticMessage("Switch statement must be exhaustive - add a default case or else pattern", switch_stmt.location, .control_flow_error));
            }
        }
    }

    /// Validate switch case
    fn validateSwitchCase(self: *Validator, case: *const SwitchCase) anyerror!void {
        try self.validateSwitchPattern(&case.pattern);
        try self.validateBlock(&case.body);
    }

    /// Validate switch pattern
    fn validateSwitchPattern(self: *Validator, pattern: *const SwitchPatternHIR) anyerror!void {
        switch (pattern.*) {
            .literal => |*expr| {
            ...
            .range => |*range| {
                ...
                // Check that range bounds are compatible types
                const start_type = self.getExpressionType(range.start);
                const end_type = self.getExpressionType(range.end);

                if (start_type != null and end_type != null) {
                    if (!start_type.?.isCompatibleWith(&end_type.?)) {
                        try self.context.addError(ValidationError.withStaticMessage("Range pattern bounds must have compatible types", range.start.getLocation(), .type_error));
                    }
                }
            },
            .enum_variant => |*variant| {
                // Check if enum and variant exist (would require type system integration)
                _ = variant;
            },
            .else_pattern => {},
        }
    ...
    fn validateExpression(self: *Validator, expr: *const Expression) anyerror!void {
        switch (expr.*) {
            ...
            .switch_expr => |*se| {
                try self.validateExpression(se.condition);
                ...
                // Validate default case if present
                if (se.default_case) |default| {
                    try self.validateExpression(default);
                }

                // Switch expressions must be exhaustive
                if (se.default_case == null) {
                    const has_else_pattern = for (se.cases) |*case| {
                        if (case.pattern == .else_pattern) break true;
                    } else false;

                    if (!has_else_pattern) {
                        try self.context.addError(ValidationError.withStaticMessage("Switch expression must be exhaustive - add a default case or else pattern", se.location, .control_flow_error));
                    }
                }
            },
        }
    ...
    fn areTypesCompatibleForBinaryOp(self: *const Validator, left: Type, right: Type, op: BinaryExpression.BinaryOp) bool {
        ...
        switch (op) {
            ...
            .comma => {
                return true; // Comma operator accepts any types
            },
        }
    }
...
pub const IRBuilder = struct {
    ...
    pub fn buildOptimized(self: *IRBuilder, ast_nodes: []AstNode) anyerror!void {
...
pub const ASTToHIRConverter = struct {
    ...
    pub fn convertAST(self: *ASTToHIRConverter, ast_nodes: []AstNode) anyerror!void {
        std.debug.print("Converting {} AST nodes\n", .{ast_nodes.len});
    ...
    fn convertASTNode(self: *ASTToHIRConverter, node: *AstNode) anyerror!void {
        switch (node.*) {
            ...
            .Import => |*import_node| {
                std.debug.print("Converting import node: {s}\n", .{import_node.path});
                try self.convertImport(import_node);
            },
            else => {
                // Skip other non-contract top-level nodes for now
                std.debug.print("Skipping non-contract node type\n", .{});
            },
        }
    }
        ...
        switch (ast_stmt.*) {
            ...
            .Log => |*log| {
                ...
                log_expr.call.callee.* = Expression{
                    .identifier = Identifier{
                        .name = IdentifierName{ .string = log.event_name },
                        .location = self.convertSourceLocation(log.span),
                    },
                };
    ...
    fn convertExpression(self: *ASTToHIRConverter, ast_expr: *ExprNode) anyerror!*Expression {
        ...
        switch (ast_expr.*) {
            .Identifier => |*ident| {
                hir_expr.* = Expression{
                    .identifier = Identifier{
                        .name = IdentifierName{ .string = ident.name },
    ...
    fn convertImport(self: *ASTToHIRConverter, import_node: *@import("ast.zig").ImportNode) anyerror!void {
        // Debug: Print import node details
        std.debug.print("=== IMPORT DEBUG ===\n", .{});
        std.debug.print("Import node - alias: '{?s}', path: '{s}'\n", .{ import_node.alias, import_node.path });
        std.debug.print("Import node alias length: {}\n", .{if (import_node.alias) |alias| alias.len else 0});
        std.debug.print("===================\n", .{});

        // Extract module name from path (e.g., "std/transaction" -> "transaction")
        const module_name = if (import_node.alias) |alias| blk: {
            std.debug.print("Using import node alias: '{s}'\n", .{alias});
            break :blk alias;
        } else blk: {
            std.debug.print("Using extracted module name from path\n", .{});
            // Hardcode for now to test
            if (std.mem.eql(u8, import_node.path, "std/transaction")) {
                break :blk "transaction";
            } else {
                break :blk extractModuleName(import_node.path);
            }
        };

        // Add the imported module to the program's imports
        try self.program.addImport(Import{
            .name = module_name,
            .path = import_node.path,
            .location = self.convertSourceLocation(import_node.span),
            .allocator = self.allocator,
        });

        // Debug: Print import info
        std.debug.print("Added import: {s} -> {s}\n", .{ module_name, import_node.path });
    }

    /// Extract module name from import path
    fn extractModuleName(path: []const u8) []const u8 {
        std.debug.print("Extracting module name from path: '{s}'\n", .{path});
        if (std.mem.lastIndexOfScalar(u8, path, '/')) |last_slash| {
            const module_name = path[last_slash + 1 ..];
            std.debug.print("Extracted module name: '{s}'\n", .{module_name});
            return module_name;
        }
        std.debug.print("No slash found, using full path: '{s}'\n", .{path});
        return path;
    }
