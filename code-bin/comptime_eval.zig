...
pub const ComptimeEvaluator = struct {
    ...
    fn parseIntegerLiteral(self: *ComptimeEvaluator, value_str: []const u8) ComptimeError!ComptimeValue {
        ...
        if (is_negative) {
            ...
            if (std.fmt.parseInt(i256, abs_str, 10)) |val| {
                return ComptimeValue{ .i256 = -val };
            } else |_| {}
        ...
        if (std.fmt.parseInt(u256, value_str, 10)) |val| {
            return ComptimeValue{ .u256 = val };
        } else |_| {}
    ...
    fn evaluateBinary(self: *ComptimeEvaluator, binary: *ast.BinaryExpr) ComptimeError!ComptimeValue {
        ...
        return switch (binary.operator) {
            ...
            .BitwiseAnd => self.bitwiseAnd(left, right),
            .BitwiseOr => self.bitwiseOr(left, right),
            .BitwiseXor => self.bitwiseXor(left, right),
            .LeftShift => self.shiftLeft(left, right),
            .RightShift => self.shiftRight(left, right),
            .StarStar => self.exponentiate(left, right),
            .Comma => right, // Comma operator returns the right operand
        };
    }
    ...
    fn exponentiate(self: *ComptimeEvaluator, left: ComptimeValue, right: ComptimeValue) ComptimeError!ComptimeValue {
        _ = self;
        return switch (left) {
            .u8 => |a| switch (right) {
                .u8 => |b| ComptimeValue{ .u8 = std.math.powi(u8, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            },
            .u16 => |a| switch (right) {
                .u16 => |b| ComptimeValue{ .u16 = std.math.powi(u16, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .u32 => |a| switch (right) {
                .u32 => |b| ComptimeValue{ .u32 = std.math.powi(u32, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .u64 => |a| switch (right) {
                .u64 => |b| ComptimeValue{ .u64 = std.math.powi(u64, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .u128 => |a| switch (right) {
                .u128 => |b| ComptimeValue{ .u128 = std.math.powi(u128, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .i8 => |a| switch (right) {
                .i8 => |b| ComptimeValue{ .i8 = std.math.powi(i8, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .i16 => |a| switch (right) {
                .i16 => |b| ComptimeValue{ .i16 = std.math.powi(i16, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .i32 => |a| switch (right) {
                .i32 => |b| ComptimeValue{ .i32 = std.math.powi(i32, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .i64 => |a| switch (right) {
                .i64 => |b| ComptimeValue{ .i64 = std.math.powi(i64, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
            ...
            .i128 => |a| switch (right) {
                .i128 => |b| ComptimeValue{ .i128 = std.math.powi(i128, a, @intCast(b)) catch |err| switch (err) {
                    error.Overflow => return ComptimeError.IntegerOverflow,
                    error.Underflow => return ComptimeError.IntegerUnderflow,
                } },
                else => ComptimeError.TypeMismatch,
    ...
    fn isBinaryOpComptimeEvaluable(self: *ComptimeEvaluator, op: ast.BinaryOp) bool {
        ...
        return switch (op) {
            ...
            .And, .Or => true,
            .BitwiseAnd, .BitwiseOr, .BitwiseXor, .LeftShift, .RightShift, .StarStar, .Comma => true,
        };
    }
...
test "comptime evaluator basic literals" {
    ...
    var int_literal = ast.LiteralNode{ .Integer = ast.IntegerLiteral{ .value = "42", .span = ast.SourceSpan{ .line = 1, .column = 1, .length = 2, .lexeme = "42" } } };
    ...
    var bool_literal = ast.LiteralNode{ .Bool = ast.expressions.BoolLiteral{ .value = true, .span = ast.SourceSpan{ .line = 1, .column = 1, .length = 4, .lexeme = "true" } } };
    ...
    // Test string literal
    var str_literal = ast.LiteralNode{ .String = ast.StringLiteral{ .value = "hello", .span = ast.SourceSpan{ .line = 1, .column = 1, .length = 7, .lexeme = "\"hello\"" } } };
