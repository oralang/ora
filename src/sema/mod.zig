const model = @import("model.zig");
const comptime_eval = @import("../comptime/mod.zig").compiler_ast_eval;
const module_graph = @import("module_graph.zig");
const region = @import("region.zig");
const resolve = @import("resolve.zig");
const type_check = @import("type_check.zig");
const verification = @import("verification.zig");
const type_descriptors = @import("type_descriptors.zig");
const abi_layout_provider = @import("abi_layout_provider.zig");

pub const ModuleImport = model.ModuleImport;
pub const ModuleGraphInput = model.ModuleGraphInput;
pub const ModuleSummary = model.ModuleSummary;
pub const NamedItem = model.NamedItem;
pub const InstantiatedStruct = model.InstantiatedStruct;
pub const InstantiatedStructField = model.InstantiatedStructField;
pub const InstantiatedEnum = model.InstantiatedEnum;
pub const ExplicitEnumValue = model.ExplicitEnumValue;
pub const InstantiatedBitfield = model.InstantiatedBitfield;
pub const InstantiatedBitfieldField = model.InstantiatedBitfieldField;
pub const TraitMethodSignature = model.TraitMethodSignature;
pub const TraitInterface = model.TraitInterface;
pub const ImplInterface = model.ImplInterface;
pub const Binding = model.Binding;
pub const ResolvedBinding = model.ResolvedBinding;
pub const GenericBindingValue = model.GenericBindingValue;
pub const GenericTypeBinding = model.GenericTypeBinding;
pub const ResolvedCall = model.ResolvedCall;
pub const TypeKind = model.TypeKind;
pub const FunctionType = model.FunctionType;
pub const IntegerType = model.IntegerType;
pub const ComptimeIntegerType = model.ComptimeIntegerType;
pub const Region = model.Region;
pub const Provenance = model.Provenance;
pub const Type = model.Type;
pub const FixedBytesType = model.FixedBytesType;
pub const LocatedType = model.LocatedType;
pub const ArrayType = model.ArrayType;
pub const SliceType = model.SliceType;
pub const AnonymousStructField = model.AnonymousStructField;
pub const AnonymousStructType = model.AnonymousStructType;
pub const anonymousStructFieldIndex = model.anonymousStructFieldIndex;
pub const anonymousStructFieldByName = model.anonymousStructFieldByName;
pub const RefinementArg = model.RefinementArg;
pub const RefinementIntegerArg = model.RefinementIntegerArg;
pub const RefinementType = model.RefinementType;
pub const ConstValue = model.ConstValue;
pub const EffectSlot = model.EffectSlot;
pub const KeySegment = model.KeySegment;
pub const Effect = model.Effect;
pub const ExprEffect = model.ExprEffect;
pub const VerificationFactKind = model.VerificationFactKind;
pub const VerificationContext = model.VerificationContext;
pub const VerificationTraitMethodOwner = model.VerificationTraitMethodOwner;
pub const VerificationStatementOwner = model.VerificationStatementOwner;
pub const VerificationFactOwner = model.VerificationFactOwner;
pub const VerificationFact = model.VerificationFact;
pub const TypeCheckKey = model.TypeCheckKey;
pub const VerificationFactsKey = model.VerificationFactsKey;
pub const ModuleGraphResult = model.ModuleGraphResult;
pub const ItemIndexResult = model.ItemIndexResult;
pub const NameResolutionResult = model.NameResolutionResult;
pub const TypeCheckResult = model.TypeCheckResult;
pub const ComptimeFormalNodeKind = model.ComptimeFormalNodeKind;
pub const ComptimeFormalEventKind = model.ComptimeFormalEventKind;
pub const ComptimeFormalEvent = model.ComptimeFormalEvent;
pub const ComptimeFoldCallSite = model.ComptimeFoldCallSite;
pub const ComptimeFoldActivation = model.ComptimeFoldActivation;
pub const ComptimeFoldDisposition = model.ComptimeFoldDisposition;
pub const ComptimeFoldEvidence = model.ComptimeFoldEvidence;
pub const ConstEvalResult = model.ConstEvalResult;
pub const VerificationFactsResult = model.VerificationFactsResult;
pub const ModuleVerificationFactsResult = model.ModuleVerificationFactsResult;
pub const appendTypeMangleName = model.appendTypeMangleName;
pub const formatEffectSlotPath = model.formatEffectSlotPath;
pub const effectSlotPathRoot = model.effectSlotPathRoot;
pub const functionHasRuntimeSelf = model.functionHasRuntimeSelf;
pub const functionRuntimeSelfParameterIndex = model.functionRuntimeSelfParameterIndex;

pub const buildModuleGraph = module_graph.buildModuleGraph;
pub const buildItemIndex = module_graph.buildItemIndex;
pub const resolveNames = resolve.resolveNames;
pub const typeCheck = type_check.typeCheck;
pub const constEval = comptime_eval.constEval;
pub const regionDisplayName = region.regionDisplayName;
pub const regionAssignable = region.regionAssignable;
pub const locatedTypeEql = region.locatedTypeEql;
pub const locatedTypeAssignable = region.isAssignable;
pub const verificationFacts = verification.verificationFacts;
pub const appendVerificationFacts = verification.appendVerificationFacts;
pub const descriptorFromTypeExpr = type_descriptors.descriptorFromTypeExpr;
pub const descriptorFromPathName = type_descriptors.descriptorFromPathName;
pub const abiLayoutProvider = abi_layout_provider.abiLayoutProvider;
