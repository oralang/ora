...
pub const StatementParser = struct {
    ...
    /// Parse statement
    pub fn parseStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        if (try self.tryParseUnlockAnnotation()) |unlock_stmt| {
            return ast.StmtNode{ .Unlock = unlock_stmt };
        }
        ...
        if (self.base.check(.At)) {
            ...
            if (self.base.check(.Identifier) and std.mem.eql(u8, self.base.peek().lexeme, "lock")) {
                ...
                return ast.StmtNode{ .Lock = .{ .path = path_expr, .span = ParserCommon.makeStmtSpan(self.base.previous()) } };
            }
        ...
        if (self.isMemoryRegionKeyword() or self.base.check(.Let) or self.base.check(.Var)) {
            ...
            if (try self.tryParseDestructuringAssignment()) |dest_assign| {
                return ast.StmtNode{ .DestructuringAssignment = dest_assign };
            }
    ...
    pub fn parseBlock(self: *StatementParser) common.ParserError!ast.BlockNode {
    ...
    fn parseVariableDecl(self: *StatementParser) !ast.VariableDeclNode {
    ...
    fn parseReturnStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    /// Parse log statement
    fn parseLogStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn parseRequiresStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn parseEnsuresStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn parseErrorDeclStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn tryParseLockAnnotation(self: *StatementParser) !?ast.VariableDeclNode {
        if (self.base.check(.At)) {
    ...
    fn tryParseDestructuringAssignment(self: *StatementParser) common.ParserError!?ast.DestructuringAssignmentNode {
        const saved_pos = self.base.current;
        ...
        var fields = std.ArrayList(ast.StructDestructureField).init(self.base.arena.allocator());
        defer fields.deinit();
        ...
        if (!self.base.check(.RightBrace)) {
            repeat: while (true) {
                ...
                try fields.append(ast.StructDestructureField{
        ...
        const pattern = ast.DestructuringPattern{ .Struct = try fields.toOwnedSlice() };

        return ast.DestructuringAssignmentNode{
            .pattern = pattern,
            .value = value_ptr,
    ...
    fn tryParseMoveStatement(self: *StatementParser) common.ParserError!?ast.MoveNode {
    ...
    fn tryParseUnlockAnnotation(self: *StatementParser) common.ParserError!?ast.UnlockNode {
        if (self.base.check(.At)) {
            const saved_pos = self.base.current;
            _ = self.base.advance(); // consume @
    ...
    fn tryParseLabeledBlock(self: *StatementParser) !?ast.LabeledBlockNode {
    ...
    fn parseExpressionStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    /// Parse if statement
    fn parseIfStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn parseWhileStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        var invariants = std.ArrayList(ast.ExprNode).init(self.base.arena.allocator());
    ...
    fn parseForStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        const pattern = if (var2) |v2|
            ast.LoopPattern{ .IndexPair = .{ .item = var1, .index = v2, .span = self.base.spanFromToken(var1_token) } }
        else
            ast.LoopPattern{ .Single = .{ .name = var1, .span = self.base.spanFromToken(var1_token) } };

        return ast.StmtNode{ .ForLoop = ast.ForLoopNode{
            .iterable = iterable,
            .pattern = pattern,
            .body = body,
    ...
    fn parseSwitchStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        return ast.StmtNode{ .Switch = ast.SwitchNode{
            .condition = condition,
    ...
    fn parseBreakStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        return ast.StmtNode{ .Break = ast.BreakNode{
            .label = label,
            .value = value,
    ...
    fn parseContinueStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
        ...
        _ = self.base.match(.Semicolon);
        return ast.StmtNode{ .Continue = ast.ContinueNode{
            .label = label,
    ...
    fn parseTryStatement(self: *StatementParser) common.ParserError!ast.StmtNode {
    ...
    fn tryParseCompoundAssignmentStatement(self: *StatementParser) common.ParserError!?ast.statements.CompoundAssignmentNode {
            ...
            self.base.match(.PercentEqual))
        {
            ...
            const lhs_ptr = try self.base.arena.createNode(ast.ExprNode);
            lhs_ptr.* = lhs;
            const rhs_ptr = try self.base.arena.createNode(ast.ExprNode);
            rhs_ptr.* = rhs;
