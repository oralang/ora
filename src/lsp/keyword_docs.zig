...
const docs = std.StaticStringMap([]const u8).initComptime(.{
    ...
    .{ "inline", "Declares a private function that is semantically expanded at each call site. Ora treats `inline fn` as source-level inlining, not as an optimizer hint." },
    ...
    .{ "resource", "Declares a nominal resource quantity domain. Storage and transient `Resource<T>` places are observed through `@amount` and updated through `@create`, `@destroy`, and `@move`." },
