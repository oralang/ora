...
pub fn prepare(
    ...
    server.response_counters.recordStringBytes(.call_hierarchy, response_stats.callHierarchyItemStringBytes(result));
    return result;
}
...
pub fn incoming(
    ...
    server.response_counters.recordStringBytes(.call_hierarchy, response_stats.incomingCallStringBytes(result));
    return result;
}
...
pub fn outgoing(
    ...
    const has_imported_members = try server.hasImportedOutgoingMembers(caller_uri);
    const workspace_entry = if (has_imported_members)
        (try server.docs.workspaceEntryForUri(caller_uri, .{ .call_edges = true, .imported_members = true }, server.workspaceRootPaths(), &server.phase_counters)) orelse return null
    else
        (try server.docs.workspaceEntryForUri(caller_uri, .calls, server.workspaceRootPaths(), &server.phase_counters)) orelse return null;
    ...
    server.response_counters.recordStringBytes(.call_hierarchy, response_stats.outgoingCallStringBytes(result));
    return result;
}
