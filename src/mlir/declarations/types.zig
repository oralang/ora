...
pub fn lowerStruct(self: *const DeclarationLowerer, struct_decl: *const lib.ast.StructDeclNode) c.MlirOperation {
    const loc = helpers.createFileLocation(self, struct_decl.span);
    ...
    // add struct name
    const name_ref = c.oraStringRefCreate(struct_decl.name.ptr, struct_decl.name.len);
    ...
    const op = self.ora_dialect.createStructDecl(struct_decl.name, loc);
...
pub fn lowerEnum(self: *const DeclarationLowerer, enum_decl: *const lib.ast.EnumDeclNode) c.MlirOperation {
    ...
    // add enum name
    const name_ref = c.oraStringRefCreate(enum_decl.name.ptr, enum_decl.name.len);
    const name_attr = c.oraStringAttrCreate(self.ctx, name_ref);
    const name_id = h.identifier(self.ctx, "sym_name");
    attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(name_id, name_attr)) catch {};
    ...
    const underlying_type = if (enum_decl.underlying_type_info) |type_info|
        ...
        c.oraIntegerTypeCreate(self.ctx, 32); // Default to i32
    const underlying_type_attr = c.oraTypeAttrCreateFromType(underlying_type);
    const underlying_type_id = h.identifier(self.ctx, "ora.underlying_type");
    attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(underlying_type_id, underlying_type_attr)) catch {};
    ...
    for (enum_decl.variants, 0..) |variant, i| {
        // add variant name
        const variant_name_ref = c.oraStringRefCreate(variant.name.ptr, variant.name.len);
        const variant_name_attr = c.oraStringAttrCreate(self.ctx, variant_name_ref);
        ...
        const variant_value = if (variant.resolved_value) |resolved|
            ...
            @as(i64, @intCast(i));
        const variant_value_attr = c.oraIntegerAttrCreateI64FromType(underlying_type, variant_value);
    ...
    if (variant_names.items.len > 0) {
        const variant_names_array = c.oraArrayAttrCreate(self.ctx, @intCast(variant_names.items.len), variant_names.items.ptr);
        const variant_names_id = h.identifier(self.ctx, "ora.variant_names");
        attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(variant_names_id, variant_names_array)) catch {};

        // add variant values array attribute
        const variant_values_array = c.oraArrayAttrCreate(self.ctx, @intCast(variant_values.items.len), variant_values.items.ptr);
        const variant_values_id = h.identifier(self.ctx, "ora.variant_values");
        attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(variant_values_id, variant_values_array)) catch {};
    }

    // add explicit values flag
    const has_explicit_values_attr = h.boolAttr(self.ctx, @as(i32, @intFromBool(enum_decl.has_explicit_values)));
    const has_explicit_values_id = h.identifier(self.ctx, "ora.has_explicit_values");
    attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(has_explicit_values_id, has_explicit_values_attr)) catch {};

    // add enum declaration marker
    const enum_decl_attr = h.boolAttr(self.ctx, 1);
    const enum_decl_id = h.identifier(self.ctx, "ora.enum_decl");
    attributes.append(std.heap.page_allocator, c.oraNamedAttributeGet(enum_decl_id, enum_decl_attr)) catch {};

    const op = self.ora_dialect.createEnumDecl(enum_decl.name, underlying_type, loc);
    for (attributes.items) |attr| {
        c.oraOperationSetAttributeByName(op, c.oraIdentifierStr(attr.name), attr.attribute);
    }
    return op;
...
pub fn createErrorType(self: *const DeclarationLowerer, error_decl: *const lib.ast.Statements.ErrorDeclNode) c.MlirType {
    ...
    _ = error_decl;
    return c.oraIntegerTypeCreate(self.ctx, 32); // Placeholder error type
}
