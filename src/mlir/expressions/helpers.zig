...
pub fn createArithmeticOp(ctx: c.MlirContext, block: c.MlirBlock, type_mapper: *const TypeMapper, _: *OraDialect, locations: LocationTracker, refinement_base_cache: ?*std.AutoHashMap(usize, c.MlirValue), op_name: []const u8, lhs: c.MlirValue, rhs: c.MlirValue, span: lib.ast.SourceSpan) c.MlirValue {
    ...
    const op = if (std.mem.eql(u8, op_name, "arith.addi"))
        ...
        c.oraSubWrappingOpCreate(ctx, loc, lhs_unwrapped, rhs_converted, lhs_type)
    else if (std.mem.eql(u8, op_name, "arith.subi"))
        c.oraArithSubIOpCreate(ctx, loc, lhs_unwrapped, rhs_converted)
    else if (std.mem.eql(u8, op_name, "ora.mul_wrapping"))
        c.oraMulWrappingOpCreate(ctx, loc, lhs_unwrapped, rhs_converted, lhs_type)
    ...
    else if (std.mem.eql(u8, op_name, "ora.shl_wrapping"))
        c.oraShlWrappingOpCreate(ctx, loc, lhs_unwrapped, rhs_converted, lhs_type)
    ...
    else if (std.mem.eql(u8, op_name, "ora.shr_wrapping"))
        c.oraShrWrappingOpCreate(ctx, loc, lhs_unwrapped, rhs_converted, lhs_type)
