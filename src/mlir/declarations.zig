...
pub const DeclarationLowerer = struct {
    ...
    /// Insert runtime guard for refinement type parameter
    fn insertRefinementGuard(
        ...
        switch (ora_type) {
            ...
            .in_range => |ir| {
                ...
                const min_const = self.createConstant(block, min_attr, value_type, loc);
                const max_const = self.createConstant(block, max_attr, value_type, loc);
    ...
    fn createConstant(self: *const DeclarationLowerer, block: c.MlirBlock, attr: c.MlirAttribute, ty: c.MlirType, loc: c.MlirLocation) c.MlirValue {
