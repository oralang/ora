...
def add_file(data: dict, source_name: str, result: dict) -> None:
    summary = data["summary"]
    ...
    for raw_loop in result.get("loops", []):
        loop = dict(raw_loop)
...
def main() -> int:
    ...
    for index, source in enumerate(sources, 1):
        ...
        add_file(report["strata"][stratum], source_name, result)
    ...
    if any(report["strata"][stratum]["tool_failures"] for stratum in STRATA):
        return 1
    return 0
