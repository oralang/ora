...
const getTrustedSetupFilepath = (kzg as any).getTrustedSetupFilepath as (
	filePath?: string,
) => string;
const DEFAULT_TRUSTED_SETUP_PATH = (kzg as any)
	.DEFAULT_TRUSTED_SETUP_PATH as string;

const TEST_SETUP_FILE_PATH_JSON = resolve(
	__dirname,
	"__fixtures__",
	"trusted_setup.json",
);
const TEST_SETUP_FILE_PATH_TXT = resolve(
	__dirname,
	"__fixtures__",
	"trusted_setup.txt",
);
...
const COMPUTE_CELLS_AND_KZG_PROOFS_TESTS =
	"../../tests/compute_cells_and_kzg_proofs/*/*/data.yaml";
const RECOVER_CELLS_AND_KZG_PROOFS_TESTS =
	"../../tests/recover_cells_and_kzg_proofs/*/*/data.yaml";
const VERIFY_CELL_KZG_PROOF_BATCH_TESTS =
	"../../tests/verify_cell_kzg_proof_batch/*/*/data.yaml";
...
type RecoverCellsAndKzgProofsTest = TestMeta<
	{ cell_indices: number[]; cells: string[] },
	string[][]
>;
...
const commitmentBadLength = new Uint8Array(
	randomBytes(BYTES_PER_COMMITMENT - 1),
);
...
const fieldElementValidLength = new Uint8Array(
	randomBytes(BYTES_PER_FIELD_ELEMENT),
);
const fieldElementBadLength = new Uint8Array(
	randomBytes(BYTES_PER_FIELD_ELEMENT - 1),
);
...
function getValidTest(testDir: string): any {
	...
	const validTest = tests.find(
		(testFile: string) =>
			!testFile.includes("invalid") &&
			!testFile.includes("incorrect") &&
			!testFile.includes("different"),
	);
...
describe("C-KZG", () => {
	...
	describe("locating trusted setup file", () => {
		it("should return a txt path if a json file is provided and exists", () => {
			expect(getTrustedSetupFilepath(TEST_SETUP_FILE_PATH_JSON)).toEqual(
				TEST_SETUP_FILE_PATH_TXT,
			);
		});
		/**
		...
		it("should return the same txt path if provided and exists", () => {
			expect(getTrustedSetupFilepath(TEST_SETUP_FILE_PATH_TXT)).toEqual(
				TEST_SETUP_FILE_PATH_TXT,
			);
		});
		describe("default setups", () => {
			beforeAll(() => {
				if (!existsSync(DEFAULT_TRUSTED_SETUP_PATH)) {
					throw new Error(
						"Default deps/c-kzg/trusted_setup.txt not found for testing",
					);
				}
			});
	...
	describe("reference tests should pass", () => {
		...
		it("reference tests for computeKzgProof should pass", () => {
			...
			tests.forEach((testFile: string) => {
				...
				const [expectedProofBytes, expectedYBytes] = test.output.map((out) =>
					bytesFromHex(out),
				);
		...
		it("reference tests for computeCells should pass", () => {
			...
			tests.forEach((testFile: string) => {
				const test: ComputeCellsTest = yaml.load(
					readFileSync(testFile, "ascii"),
				);
		...
		it("reference tests for computeCellsAndKzgProofs should pass", () => {
			...
			tests.forEach((testFile: string) => {
				const test: ComputeCellsAndKzgProofsTest = yaml.load(
					readFileSync(testFile, "ascii"),
				);
		...
		it("reference tests for recoverCellsAndKzgProofs should pass", () => {
			...
			tests.forEach((testFile: string) => {
				const test: RecoverCellsAndKzgProofsTest = yaml.load(
					readFileSync(testFile, "ascii"),
				);
				...
				try {
					[recoveredCells, recoveredProofs] = recoverCellsAndKzgProofs(
						cellIndices,
						cells,
					);
				} catch {
		...
		it("reference tests for verifyCellKzgProofBatch should pass", () => {
			...
			tests.forEach((testFile: string) => {
				const test: VerifyCellKzgProofBatchTest = yaml.load(
					readFileSync(testFile, "ascii"),
				);
	...
	describe("edge cases for blobToKzgCommitment", () => {
		describe("check argument count", () => {
			const test: BlobToKzgCommitmentTest = getValidTest(
	...
	describe("edge cases for computeBlobKzgProof", () => {
		describe("check argument count", () => {
			const test: ComputeBlobKzgProofTest = getValidTest(
	...
	describe("edge cases for verifyBlobKzgProof", () => {
		describe("check argument count", () => {
			const test: VerifyBlobKzgProofTest = getValidTest(
	...
	describe("edge cases for verifyBlobKzgProofBatch", () => {
		describe("check argument count", () => {
			const test: VerifyBatchKzgProofTest = getValidTest(
