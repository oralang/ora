 ...
 */
bindings.DEFAULT_TRUSTED_SETUP_PATH = path.resolve(
	__dirname,
	"..",
	"deps",
	"c-kzg",
...
bindings.getTrustedSetupFilepath = function getTrustedSetupFilepath(filePath) {
	if (filePath) {
		...
		if (!fs.existsSync(filePath)) {
			throw new Error(
				"Default trusted setup not found. Must pass a valid filepath to load c-kzg library",
			);
		}
	}
...
bindings.loadTrustedSetup = function loadTrustedSetup(precompute, filePath) {
	originalLoadTrustedSetup(
		precompute,
		bindings.getTrustedSetupFilepath(filePath),
	);
};
